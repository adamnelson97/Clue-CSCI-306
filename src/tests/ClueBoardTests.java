...
public class ClueBoardTests {
	...
	public void testDoorwayDirections() {
		...
		BoardCell door = board.getCellAt(2, 4); // Kitchen door (Down)
		assertEquals(DoorDirection.DOWN, door.getDoorDirection());
		door = board.getCellAt(2, 14); // Ballroom door (Left)
		assertEquals(DoorDirection.LEFT, door.getDoorDirection());
		door = board.getCellAt(22, 3); // Library door (Right)
	...
	public void testRoomInitials() {
		...
		cell = board.getCellAt(0, 10); // Conservatory (C)
		assertEquals('C', cell.getInitial());
		cell = board.getCellAt(0, 19); // Ballroom (B)
		assertEquals('B', cell.getInitial());
		cell = board.getCellAt(12, 0); // Servant's Quarters (Q)
		assertEquals('Q', cell.getInitial());
		cell = board.getCellAt(11, 19); // Art Room (A)
		assertEquals('A', cell.getInitial());
		cell = board.getCellAt(16, 19); // Trophy Room (R)
		assertEquals('R', cell.getInitial());
		cell = board.getCellAt(19, 0); // Library (L)
		assertEquals('L', cell.getInitial());
		cell = board.getCellAt(20, 11); // Theatre (T)
		assertEquals('T', cell.getInitial());
		cell = board.getCellAt(20, 19); // Master Bedroom (M)
